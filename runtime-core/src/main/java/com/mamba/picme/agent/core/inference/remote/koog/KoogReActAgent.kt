 ...
 * - **纯协程**：[executeTask] 在内部 [CoroutineScope]（SupervisorJob + Dispatchers.IO）launch 一个 Job，
 *   替代旧的单线程 executor；[cancel] = 取消当前 Job + cancelled 标志（Koog 1.1.1 已正确响应协程取消，
 *   agent.run 抛 CancellationException → 走 onComplete(0, "Task cancelled", ...) 分支，语义对齐旧实现）。
 * - **记忆**：Koog ChatMemory feature（**不**设 windowSize）+ [KoogSessionHistoryProvider]（包
 *   [KoogMessageMemoryStore]），三不变式（System 不落盘 / tool_call 块原子裁剪 / 双向配对剔除）由 store
 *   强制；sessionId 语义对齐旧行为（飞书默认 "feishu_p2p"，相机由 [setSessionId] 切 "camera" 等；
 *   历史不迁移，新键前缀 `koog_memory_` 自然隔离）。
...
class KoogReActAgent(
    ...
    private val toolService: ToolSet? = null,
) {
    private val tag = "KoogReActAgent"

    // 飞书（远程控制 RPA）默认用 RemoteControlToolService(windowManager)；相机注入 CameraToolService 单例。
    // 当 toolService=null 时要求 windowManager 非 null（飞书路径，语义对齐旧 RemoteReActAgent）。
    private val effectiveToolService: ToolSet =
    ...
    /** DataStore 持久化记忆（键前缀 koog_memory_，与旧 langchain4j 路径的 memory_ 隔离，历史不迁移）。 */
    private val memoryStore by lazy {
        val ctx = appContext
            ?: throw IllegalStateException("No context available for KoogMessageMemoryStore")
        KoogMessageMemoryStore(ctx)
    }
     ...
     * 读取；同时注入给 CameraToolService/ChatToolService（dispatchCommand 读它注入 AgentContext）。
     */
    private val traceIdHolder = TraceIdHolder()

    init {
        // 相机路径（CameraToolService）共享同一 holder：dispatchCommand 读取当轮 traceId 注入
        // AgentContext，使远程 ReAct 下的 tool 执行也带 traceId，与 LLM 调用日志关联。
        // 飞书路径（RemoteControlToolService）非 chat/相机来源，不匹配跳过（语义对齐旧实现）。
    ...
    /** LlmCallRecord 来源标签：相机/飞书链路区分（chat 链路是 KoogChatAgent 的 "chat-koog"）。 */
    private val recordSource: String =
        if (effectiveToolService is CameraToolService) "camera-koog" else "feishu-koog"

    private val running = AtomicBoolean(false)
    private val cancelled = AtomicBoolean(false)

    /** 任务执行作用域：SupervisorJob 隔离单任务失败；Dispatchers.IO 承载网络/工具等待。 */
    ...
    // 单 run 累计状态（runs 串行：running 互斥守卫）。StringBuffer/Atomic 线程安全，
    ...
    private suspend fun runTask(userPrompt: String, cb: RemoteReActAgentCallback, traceId: String?) {
        ...
        try {
            ...
            Logger.i(tag, "Task complete: result='$summary', latency=${metrics.latencyMs}ms, " +
                "prompt=${promptTokens.get()}, completion=${completionTokens.get()}")
    ...
    private fun totalTokens(): Int = promptTokens.get() + completionTokens.get()

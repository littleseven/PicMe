...
class GalleryScriptHandlersTest {
    ...
    private fun newFixture(scope: CoroutineScope): Fixture {
        ...
        return Fixture(runtime, engine.bridge, personDao, queryGalleryMediaUseCase)
    }
    ...
    @Test
    fun `registers all 10 handlers including face_cluster and tag_audit`() = runTest {
        val f = newFixture(this)
        val expected = setOf(
            "gallery.summary", "gallery.query", "gallery.tags", "gallery.timeline",
            "gallery.intersect", "gallery.stats_by_tag", "media.meta", "media.batch_meta",

...
| 产品线 | 定位 | 核心价值 | 研究目标 |
|--------|------|----------|----------|
| **相册及编辑** | **核心产品与默认首页** | 本地 AI 修图 + 精准局部控制 + 自然语言找图 | 美颜算法离线效果评估、编辑体验研究 |
| **AI 对话** | **相册内的助手能力** | 自然语言交互 + 模型切换 + 对话持久化 | 端侧模型对话体验、人机交互范式验证 |
| **拍照** | 能力验证基线 / 内容采集入口 | 零延迟 + 自然美颜 | 实时渲染管线、人脸关键点驱动美颜 |
| **视频** | 技术挑战场景 | 实时美颜录制 + 帧同步稳定 | GPU 管线在录制场景的极限验证 |
| **IM 远程控制** | **实验性融合入口（P2）** | IM + LLM 远程操控 + 飞书机器人接入 | 验证跨平台远程 App 控制场景的可行性 |

```
相册+编辑（核心产品与默认首页）→ AI 对话（相册内助手能力）→ 拍照（内容采集入口）→ 视频（极限挑战）
                              ↓
...
### 6.6 IM 远程控制线（IM Remote Control）— 实验性融合入口（P2）

> **定位调整**：IM 远程控制从 P0 降级为 P2 实验线，不占用核心产品研发资源，仅在主路径稳定后投入。
...
| 阶段 | 时间 | 目标 | 关键交付 |
|------|------|------|----------|
| **Phase 1** | 近期（2-4 周） | 相册首页成为默认入口 + 编辑核心体验 | 相册首页默认化；AI 助手入口；AI 一键优化 + 精准局部美颜；对话持久化；模型状态可视化；相机入口迁移至相册底部 Tab |
| **Phase 2** | 中期（4-8 周） | 智能体验升级 + 对话式编辑 | 图片消息支持；对话式编辑；智能消除；专业调色；自然语言搜索；实时美颜录制（1080p@30fps）；IM 远程控制实验启动（飞书 SDK 集成 + 设备绑定 + 基础命令） |
| **Phase 3** | 长期（8-16 周） | 创作者生态 + 视频进阶 | 多会话管理；证件照/连拍/RAW；智能运镜/Vlog/多平台导出；拼图/Pro 功能；IM 远程控制根据实验数据决定是否继续投入 |

**资源分配建议**：
- Phase 1：编辑 50% / 对话 35% / 拍照 10% / IM 远程 5% / 视频 0%（核心资源投向相册首页与编辑体验）
- Phase 2：编辑 45% / 对话 35% / 视频 10% / IM 远程 10% / 拍照 0%（IM 远程控制启动实验）
- Phase 3：编辑 40% / 对话 25% / 视频 20% / IM 远程 10% / 拍照 5%（根据实验数据调整 IM 投入）

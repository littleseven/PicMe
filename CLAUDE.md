...
**Current focus (2026-08, app v1.0.34)** is the smart gallery as the default home with AI chat as the core assistant capability (相册/图片编辑为主入口, camera as auxiliary). Shipped: natural-language search, conversational image editing, matting/ID-photo, Florence-2 auto-tagging, JS sandbox, fact memory + person-relationship graph (capabilities live). See `PRODUCT.md` for the latest product roadmap.

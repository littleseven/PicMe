...
| 2026-08-16 | **更正 Arbot 定位：预览层，非 spec SSOT** | 「画布作 SSOT」路线已于 2026-08-15 随 codegen 工作流废止（`design-tokens.json` 为唯一 SSOT，双端镜像由 `gen-design-tokens.py` 生成 + `ai-gate.sh --check` 门禁）；Arbot/Figma 画布降级为**可视化预览层**。上行的「polang-ui-spec.ardot 成为 Camera spec SSOT」表述作废——实际画布文件为根目录 `polang-ui-spec.fig`（untracked）。历史决策见 `docs/superpowers/specs/2026-08-14-figma-ui-spec-completeness-design.md` 顶部废止横幅 |

...
| 2026-08-08 | K3 相册 UI 按 Android 原貌整体重做（依据 `docs/reviews/2026-08-08-ios-gallery-ui-gap-analysis.md` 47 项对照）：网格页去 NavigationStack 改自建 48pt AppTopBar + 操作组（分组菜单 NONE/DATE 实做，模型中心/扫描/搜索/设置 Phase 6 管线依赖灰置）+ adaptive(110) 列 + GroupHeaderView + 冷启动 SplashPlaceholder（365 条格言池从 GalleryQuotes.kt 全量移植）+ 空态/权限四态 PermissionMessageView；缩略图人脸感知对齐（FaceAwareAlignment 公式逐行移植，数据源待 Phase 6）+ 视频标记 + 圆角；大图页去 page dots + 1–4x 缩放/平移（clamp 公式同 ZoomableImage）+ 单击切换顶/底栏 + 顶栏（返回/日期/Info）+ 底栏（分享/删除，编辑/证件照 Phase 6 不假造）+ PhotoInfoSheet 简版；xcstrings 补 24 键三语对齐 Android strings.xml。xcodebuild build + test 12/12 全绿。commits：43c7999d / 34b0ddc4 / dd99eb36。遗留差异项（见差距分析 §5）：FloatingBottomTab、选择模式/拖拽多选、视频播放、转场动画、搜索/TAG/模型中心管线、FACE 等分组数据源、编辑/证件照、完整 PhotoInfo |
| 2026-08-09 | K3 相册 UI 视觉参照对齐轮（地面真值闭环，基准 `tmp/ui-reference/android/` dump + `IOS_ANDROID_UI_PARITY.md` 方法论）：dump 实测修正上轮按代码分析的重做——固定 3 列（去 adaptive，列宽按比例导出：128.3pt≈32.6% 屏宽 = Android 117.4dp/360dp）、分组头计数"(N)"右对齐（上轮误为标题相邻）、顶栏 5 按钮触控框 36pt/pitch 43pt 对齐 dump、Menu 内建 padding 挤出修复（`.frame` 限定 + `.menuIndicator(.hidden)`）、大图页顶栏 48pt 按钮/更多菜单三项 disabled、底栏 4 位 SpaceEvenly（编辑/证照灰置占位）、选择模式全量落地（长按进入/勾选圈+浅蓝遮罩/顶栏 morph/全选/ShareSheet 分享/PhMediaBridge 删除+确认）、分组下拉扁平 6 项（FACE 等无数据源者灰置）。**letterbox 黑带根因修复**：`project.yml` 补 `INFOPLIST_KEY_UILaunchScreen_Generation=YES`（缺失导致 App 以 320×480pt 兼容模式渲染、上下各 132pt 黑带——§1.3 系统栏登记翻车点），修复后三态截图全屏白/黑底符合登记。归一化抽检：格宽/gap 2.0pt(Android 2.1dp)/边距/栏高全部命中。模拟器闭环：XCUITest 授权通道（`simctl privacy grant` 对 iOS 18.6 readWrite 不生效，已废设备 83445D83；新设备 2E3911EB 授权 full）+ launch 参数钩子截图，临时产物（PoLangUITests target/钩子）已 revert。xcodebuild test 12/12 绿 + `:shared:iosX64Test` 25 suite 全绿。剩余登记差异：TAG ProgressBar/底 tab 胶囊（GLM 域 MainTabView）/FACE 等分组数据源/编辑证照功能/完整 PhotoInfo |

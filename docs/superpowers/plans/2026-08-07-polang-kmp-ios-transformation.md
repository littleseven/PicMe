...
- 🔄 **6.1 TAG 流水线**（基建层已建，端到端未组装）：✅ **Pass1 基建已落地**——`Pass1Pipeline.swift` 编排器（对标 Android `TagGenerationPipeline.stage1WithEmbeddings`）+ `FaceAlignment` 仿射对齐 + `MobileClipEncoder` 语义 + `TagDatabase`(GRDB) + 人脸检测可用（MNN 2d106det 预归一化修复，self-test 106pt 绿；MediaPipe 468→106 适配），commit `25414e12` Step1-6 完成。❌ **未组装**：Pass2 聚类（DBSCAN/kNN 连通分量）；Pass3 VLM 打标（🔴 阻塞于补验 B「Qwen3-VL-2B 真机」暂缓，恢复触发点 = Pass3 开工前）；**iOS MetalGuardian** 新设计（替代 OpenClGuardian——iOS 无 OpenCL：Metal kernel warmup 超时 + Metal→CPU 降级含模型卸载重载 + MTLDevice 丢失处理 + 黑名单持久化）；3-Pass 控制页 + 扫描编排（`Pass1Pipeline` 当前未接线任何 UI/VM）；后台扫描（FGS→BGTaskScheduler，iOS ~30s 限制→改增量/手动）。4 模型（RetinaFace/R100/MobileCLIP-S2/Qwen3-VL-2B）顺序加载内存峰值待 Instruments 验证。
...
- [ ] **6.6 功能深化对齐**（2026-08-09 增补）：相机/gallery 规格 gap 落地 + 人物页 + 自然语言搜索。源自 `specs/2026-08-09-ios-spec-test-gaps.md` 与 Phase 5 Task20/21 登记差异。切片：
  - **相机域**：唇彩/腮红 makeup（§8）、5 款风格滤镜（§9 TOON/SKETCH/POSTERIZE/EMBOSS/CROSSHATCH）、比例/场景/网格/ProMode 面板（§10–13）、VIDEO/DOCUMENT 模式（§7）、构图网格叠加（§6）、人脸联动对焦框（§5）、语音/AI Chat FAB（§14）
...
> **状态同步（2026-08-10 回写）**：6.2 ✅、6.4 ✅、6.3 🔄（主体完成剩 3 项）、6.5 🔄（部分）、6.1 🔄（Pass1 基建已建，端到端未组装；Pass3 阻塞于补验 B 暂缓）、6.6 🔄（人物页 UI 已落地，相机/gallery/搜索待开工）。✅ main 已与 origin/main 同步（~~44 commits 未 push~~ 已全推）。

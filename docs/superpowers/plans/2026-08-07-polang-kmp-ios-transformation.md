...
- [ ] **6.1 TAG 流水线**（未启动，阻塞项）：MNN 推理接入（Phase 2.1 产物；🔴 补验 B「Qwen3-VL-2B 真机」暂缓，恢复触发点 = 本子项开工前）；3-Pass 控制页；新设计 **iOS MetalGuardian**（替代 OpenClGuardian——iOS 无 OpenCL，非「策略对齐」而是新设计：Metal kernel warmup 超时检测 + Metal→CPU 降级含模型卸载重载 + MTLDevice 丢失处理 + 黑名单持久化）；4 模型（RetinaFace/R100/MobileCLIP-S2/Qwen3-VL-2B）顺序加载内存峰值用 Instruments 验证
...
- [ ] **6.6 功能深化对齐**（2026-08-09 增补）：相机/gallery 规格 gap 落地 + 人物页 + 自然语言搜索。源自 `specs/2026-08-09-ios-spec-test-gaps.md` 与 Phase 5 Task20/21 登记差异。切片：
  ...
  - **人物页**（Phase 5 Task20/21 登记「未实现」）：人物记忆与关系图谱，消费 shared `PersonRelationCapability` / `MemoryCapability`
  - **自然语言搜索**（依赖 6.1 TAG 索引）：Android `ExplicitFirstSearchPipeline` / `QueryParser` 对齐

> **状态同步（2026-08-09）**：6.2 ✅、6.4 ✅、6.3 🔄（主体完成剩 3 项）、6.5 🔄（部分）、6.1 未启动（阻塞于补验 B 暂缓）、6.6 待开工。⚠️ main 本地领先 origin/main **44 commits 未 push**（Phase 5/6 全部成果在本地）。

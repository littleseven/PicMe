...
- [ ] **6.5 验收对齐**（部分完成）：iOS UI Driver + 截图自动验收流程 ✅（`79350b84`，launch arg `-startPage` + `PoLangUITests`）；`docs/08-UI-SPECS/screens/*.yaml` 规格→UITest gap 清查完成（`specs/2026-08-09-ios-spec-test-gaps.md`）。**剩余**：① 核心验收 UITests 补全（media_pager 标识符修复 + 相机/gallery 控件 identifier，见 gap 清单）；② 结构化日志（llm/tool/js 三层）iOS 落地（未启动）

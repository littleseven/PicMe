...
> **定位**：Cloudflare 是「免费的、迟早要上的加速与保险」，**不是 Day-1 阻塞项**。MVP 可以完全不用 Cloudflare 直接跑通（见 §5.0）；等准备扩大暴露面、或需要抗 D / 零下行费对象存储时再加，改动很小、收益很大。把它放在源站前面，等于免费获得全球 CDN、企业级抗 D、托管 TLS 与对象存储。

### 5.0 无 Cloudflare 的最小部署（Day-1 可用）

验证阶段完全可不引入 Cloudflare，一台 HK VPS 直跑即可对外提供 HTTPS API：

```
用户 ──HTTPS──▶ 香港轻量 VPS（公网 IP）
                 ├─ Caddy（自动申请/续期 Let's Encrypt 证书，反代后端）
                 ├─ 单体后端（Ktor/Go）+ SQLite(WAL)
                 └─ 模型 / 滤镜 / 预设 直接放磁盘，由后端 /assets 静态返回
   域名 A 记录直接指向 VPS；ufw 仅开 443 + 受限 SSH（改端口 + 公钥登录）
```
...
| 项 | 做法 | 说明 |
|----|------|------|
| TLS | Caddy 自动签 Let's Encrypt（免费、自动续期） | 无需 Cloudflare |
| DNS | 注册商自带 / 任意免费 DNS | A 记录 → VPS 公网 IP |
| 模型分发 | 放 VPS 磁盘，后端 `/assets/*` 静态返回 | 低流量足够；后续迁 R2 |
| 限流 / 安全 | 后端基础限流 + ufw 防火墙 | 够 MVP |
| 备案 | 香港机房免备案 | 直跑 443 无障碍 |

**何时迁出最小部署、引入 Cloudflare**：流量增长占满 VPS 带宽/流量包、需要抗 DDoS 或隐藏源站 IP、要零下行费对象存储（R2）、要托管 TLS 或边缘限流/WAF 时——见 §5.1 起。

> **权衡提醒**：若一开始裸跑，源站公网 IP 一旦被扫描记录，后期套 Cloudflare 后该 IP 仍可被直连（需换 IP 或源站改为「仅放行 CF 回源 IP」才能彻底隔离）。MVP 无公开画像时被针对性攻击概率低；**建议在「扩大暴露面之前」补上 Cloudflare**。
...
**结论**：对一台 2C2G 的小机器，Cloudflare 能把「面向公网稳定服务」这件事变得轻松且近乎免费——但它**不是 Day-1 必需**（见 §5.0 的最小部署）。推荐节奏：先裸跑验证，**在扩大暴露面前补上 Cloudflare**。

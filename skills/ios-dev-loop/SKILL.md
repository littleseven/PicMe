...
name: ios-dev-loop
description: |
  iOS 真机无人值守闭环验证：工程准备→编译→安装→启动→设备验证→报告（5 阶段、阶段隔离、自动设备检测）。Use when verifying an iOS change end-to-end on a real device unattended, checking for black screens / crashes, or running cross-platform Android↔iOS pixel diff via scripts/ios-auto-dev-loop.sh.
version: 2.0.0
created: 2026-08-08
updated: 2026-08-09
maintainer: "[RD] 全栈工程师"
...
| 能力 | 说明 |
|------|------|
| **自动设备检测** | 物理真机 UDID（`8hex-16hex`）自动识别；`--device ID` 或 `IOS_DEVICE_ID` 环境变量覆盖 |
